$(document).ready(function() {//----- Ejecutar funciones una vez cargar la pagina HTML
   ...
   function getSeguros(id_medico) {//declara la funcion especialidades
    $.ajax({
        url: '../php/aseguradora/seguros_med.php',//abrir el archivo especialidades-list.php sin parametro
        type: 'POST',
        async: false,
        ...
        success: function(response) {
            ...
            $('#select_seguros').html(template);// carga en el select todos los option generados
       ...
       $.ajax({ //mediante AJAX hacemos una consulta hacia la base de datos mediante un archivo php
           ...
           success: function(response) { //Cuando el archivo de destino responde de manera positiva se recibe un resultado response
               if (response != false) {//Evalua si el response no es false
                   ...
                   {
                    $('#texto_modal').html('Paciente pertenece al seguro: '+paciente.seguro);
                    $('#modal_icon').attr('style', "color: rgb(220, 175, 13)");
